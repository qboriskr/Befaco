...
public:
...
#ifdef VCV
        // Under VCV there is no Owl patch wrapper to query switch states from;
        // the port drives these directly.
        patchCtrls_->looperResampling = 0.f;
        patchCtrls_->oscUseWavetable = 0.f;
#else
    ...
    void LoadConfig() {
#ifdef VCV
        // VCV: there is no oneiroi.cfg resource, use the firmware's default
        // calibration values (same as the else branch below).
        hwRevision_ = 0;
    ...
    // VCV accessors.
    Led* GetLed(LedName led) { return leds_[led]; }
    bool IsShiftOn() { return shiftButton_->IsOn(); }
    bool IsModCvOn() { return modCvButton_->IsOn(); }
