...
static const float kHPi = kPi * 0.5f;
constexpr float kSqrt2 = 1.414213562373095f;
static const float kRSqrt2 = 1.f / kSqrt2;
 ...
 * @return float
 */
// VCV: internal linkage so Iroi and Oneiroi TUs don't clash at link time.
static float Power(float f, float n = 2, bool approx = false)
{
    if (approx)
    {
        long *lp, l;
...
// VCV: internal linkage so Iroi and Oneiroi TUs don't clash at link time.
static void CheapEqualPowerCrossFade(AudioBuffer &from, AudioBuffer &to, float pos, AudioBuffer &out, float p = kEqualCrossFadeP)
{
    float invPos = 1.f - pos;
    float k = -6.0026608f + p * (6.8773512f - 1.5838104f * p);
    float a = pos * invPos;
 ...
 */
// VCV: internal linkage so Iroi and Oneiroi TUs don't clash at link time.
static void SquareTable(float *tab, uint32_t sz, uint32_t p = 0, float a = 1.f, float d = 0.5f, uint32_t o = 0)
{
    if (sz <= 0) return;
    if (p <= 0)
    {
...
// VCV: internal linkage so Iroi and Oneiroi TUs don't clash at link time.
static float HardClip(float x, float limit = 1.f)
{
    return Clamp(x, -limit, limit);
}

// VCV: internal linkage so Iroi and Oneiroi TUs don't clash at link time.
static float AudioClip(float x, float s = 1.f)
{
    return SoftClip(x * s);
}
...
// These are taken and adapted from code found in Emilie Gillet's eurorack repo.
// VCV: internal linkage so Iroi and Oneiroi TUs don't clash at link time.
